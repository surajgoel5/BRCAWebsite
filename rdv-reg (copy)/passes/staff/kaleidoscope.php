...
$request=$con->query("INSERT INTO `".$category."kaleidoscope` (kerberos,name,relation1,relation2,relation3,time) VALUES ('$kerberos','$name','$relation1','$relation2','$relation3',now())");
if($request)
    die('{"success":true,"message":"Request added"}');
else
    $request=$con->query("UPDATE `".$category."kaleidoscope` SET relation1='$relation1',relation2='$relation2',relation3='$relation3',time=now() where kerberos='$kerberos'");
